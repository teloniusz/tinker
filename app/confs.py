...
ConfTypeName = Literal['list', 'dict', 'str', 'int', 'float', 'bool', 'any', 'struct', 'scalar']
...
def is_expected_type(val: ConfVal, expect: ConfTypeName):
    ...
    try:
        ...
        if expect == 'any':
            return True
        # scalar or struct
        return (expect == 'struct') == isinstance(val, (list, dict))
...
class ConfDict(dict[str, ConfVal]):
    ...
    def __getitem__(self, key: str | tuple[ConfTypeName, str]) -> ConfVal:
    ...
    def get_as(self, expected_type: ConfTypeName, key: str, default: _T = None) -> ConfVal | _T:
        try:
            res = super().__getitem__(key)
        except KeyError:
            return default
